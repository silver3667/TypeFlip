...
Uses standard Israeli SI-1452 layout mapping.
"""
...
ENGLISH_TO_HEBREW = {
    ...
    "=": "=",
    "\\": "\\",
    # Uppercase (shift + key) - Hebrew layout often keeps Latin for shift
    "Q": "/",
    "W": "'",
    "E": "ק",
    "R": "ר",
...
for k, v in ENGLISH_TO_HEBREW.items():
    if k.isalpha() and k.isupper():
        continue  # skip uppercase so we get lowercase output for letters
    if v not in HEBREW_TO_ENGLISH:
        HEBREW_TO_ENGLISH[v] = k
# Prefer semicolon for geresh
HEBREW_TO_ENGLISH["׳"] = ";"

# Hebrew letters (Unicode range for Hebrew is 0x0590-0x05FF, plus punctuation)
HEBREW_LETTERS = set(
    "אבגדהוזחטיכסעפצקרשת"
    "ךםןףץ"
    "׳"
